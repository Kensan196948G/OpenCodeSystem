#!/usr/bin/env bash
# Monitor: CTO監視フェーズ
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "$SCRIPT_DIR/config.sh"

PROJECT_PATH="$1"
PROJECT_NAME=$(basename "$PROJECT_PATH")

log "monitor" "Starting monitor for $PROJECT_NAME"

PROMPT_FILE="$SCRIPT_DIR/agents/cto.md"
PROMPT=$(cat "$PROMPT_FILE")
PROMPT="${PROMPT//\{\{PROJECT_NAME\}\}/$PROJECT_NAME}"
PROMPT="${PROMPT//\{\{PROJECT_PATH\}\}/$PROJECT_PATH}"

cd "$PROJECT_PATH"
if $OPENCODE run --model "$MODEL_PRO" --dangerously-skip-permissions "$PROMPT" 2>&1; then
  log "monitor" "Monitor analysis completed"
  exit 0
else
  log "monitor" "Monitor analysis failed"
  exit 1
fi
